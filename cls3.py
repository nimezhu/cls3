...
def Main():
    ...
    if args.M:
        ...
        print("",file=out)
        for i,xs in enumerate(zip(tsneY,idxt)):
            x=xs[1]
            tx,ty=xs[0]
...
def read_matfile(f,header=1,col=1,name_col=0,transpose=False):  #start col 0
    ...
    for i in range(header):
        headers.append(iter.__next__());
    for i in iter:
        names.append(i[name_col]);
        mat.append([float(a) for a in i[col:]])
    if len(headers)==0:
        headers.append([str(f)+".V"+str(j+1) for j in range(len(mat[0]))]);
    if transpose:
        mat=np.array(mat)
        tmat=mat.T
...
def akmeans(S,kmethod,ro):
    ...
    if kmethod==1:
        ...
        diff=[i-j for i,j in zip(value,Pvalue)]
        print("diff =",diff)
    rank=range(0,l)
    print("value=",value) 
    max_value=value[-1]
    ...
    if kmethod==2:
        ...
        for i,j in zip(value[::-1],Pvalue[::-1]):
                if i0:
                    i0=False
                    continue
...
def dist(mu,idx,obs):
    MIN_DIST=0.01
    dist=[0.0 for i in range(mu.shape[0])]
...
def davies_bouldin_index(mu,dist):
    ...
    n=len(mu)
    for i in range(n):
        db+=max(array([(sqrt(dist[j])+sqrt(dist[i]))/euclidean(mu[i],mu[j]) for j in range(n) if j!=i]))
    db*=1.0/n
    return db
    db=0.0
    n=len(mu)
